class DateUtils {
  ...
  static int calculateMaxWeeksNumberMonthly(
    ...
    if (monthsNumber == 0) {
      ...
      int weeks = weeksNumbersMonthly[0];

      return 6;
    }
  }

  static int calculatorWeeksInMonth(DateTime startDate){
    int weeks = 4;
    int firstDay = startDate.weekday;
    int daysInWeek = getLastDayOfMonth(startDate).day;
    if(daysInWeek == 31){
      if(firstDay < 6) {
        weeks = weeks + 1;
      }else{
    ...
    }else if(daysInWeek == 30){
      if(firstDay == 7) {
        weeks = weeks + 2;
      }else {
        weeks = weeks + 1;
      }
    }else if(daysInWeek == 29){
      weeks = 5;
    }else if(daysInWeek == 28){
      if(firstDay == 1) {
        weeks = weeks;
      }else  {
        weeks = 4 + 1;
